...
internal class FaceAnalyzer(
    ...
    private val successListener = OnSuccessListener<List<Face>> { faces ->
        ...
        if (face != null) {
            if (detectStatus == FaceAnalyzerStatus.UnDetect) {
                ...
                listener?.detect()
                listener?.detectProgress(25F, "얼굴을 인식 했습니다. \n 왼쪽 눈만 깜빡여 주세요.")
                ...
                listener?.detectProgress(55F, "오른쪽 눈만 깜빡여 주세요.")
                ...
                listener?.detectProgress(100F, "얼굴 인식이 완료 되었습니다.")
                listener?.stopDetect()
                detector.close()
            }
            calDetectSize(face)
    ...
    private fun calDetectSize(face: Face) {
        ...
        if (abs(preCenterX - centerY) > PIVOT_OFFSET
